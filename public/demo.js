...
form.addEventListener('submit', async event => {
  ...
  try {
    const response = await fetch(new URL('/api/emitir-factura', apiUrl), {
    ...
    status.textContent = error instanceof TypeError || error.name === 'TimeoutError'
      ? 'No se pudo conectar con Faspy. Comprueba que esté en puerto 3000, abre este ERP con localhost:3001 y revisa ERP_ORIGIN en el core.'
      : error.message;
